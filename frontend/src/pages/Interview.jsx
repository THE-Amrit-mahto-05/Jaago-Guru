  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <div className="flex-1 p-10">
        ...
        <div>
      <h1 className="text-4xl font-bold text-slate-900">Mock Interview Setup</h1>
<p className="text-slate-600 mt-1 text-lg"> Configure your interview parameters</p></div>
</div> </div>
...
<div className="flex items-center gap-3 mb-2">
 ...
 <h3 className="font-semibold text-slate-900">Level-Matched</h3></div>
...
<div className="flex items-center gap-3 mb-2">
  <div className="w-10 h-10 bg-blue-50 rounded-lg flex items-center justify-center">
<Hash className="text-blue-600" size={20} /></div>
...
<h2 className="text-2xl font-bold text-slate-900 mb-6"> Interview Configuration</h2>
 <div className="space-y-6">
<div className="relative w-full">
   <label className="block mb-2 text-sm font-semibold text-slate-700 uppercase tracking-wide">Job Role </label>
 <div onClick={() => setOpen(!open)}
 className={` bg-gradient-to-br from-white to-blue-50/30 p-4 rounded-xl border-2 
 ${open ? 'border-blue-500 shadow-lg' : 'border-slate-200'} cursor-pointer transition-all duration-200 flex justify-between items-center hover:border-blue-300  `} >
...
stroke="currentColor"
 ...
 {open && (  <div className="absolute left-0 right-0 mt-2 bg-white border-2 border-slate-200 shadow-xl rounded-xl overflow-hidden z-50 max-h-80 overflow-y-auto">
 {jobRoles.map((item, idx) => ( 
  <div key={idx} onClick={() =>
     {setRole(item);
     setOpen(false);}}
 className={` px-4 py-3 cursor-pointer font-medium transition-all  ${role === item  ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white' : 'text-slate-700 hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50'
 }`} >
{item}
</div>))}
 </div> )}
